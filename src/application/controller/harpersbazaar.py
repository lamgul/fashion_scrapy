...
harpersbazaar_router = APIRouter(tags=['HarpersBazaar'], prefix="/api")


@harpersbazaar_router.get(path="/harpersbazaar")
...
@harpersbazaar_router.get(path="/harpersbazaar/{trace_id:str}")
def harpersbazaar_list(
        trace_id: str = Path(),
        usecase: USECASE_TYPE = Depends(HarpersBazaarUsecase)
):
...
@harpersbazaar_router.post(path="/harpersbazaar/{trace_id:str}/convert")
