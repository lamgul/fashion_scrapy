...
kmodes_router = APIRouter(tags=['Kmodes'], prefix="/api")


@kmodes_router.get(path="/kmodes")
def kmodes_list(
...
@kmodes_router.get(path="/kmodes/{trace_id:str}")
...
@kmodes_router.post(path="/kmodes/{trace_id:str}/convert")
def kmodes_convert(
        trace_id: str = Path(),
        usecase: USECASE_TYPE = Depends(KmodesUsecase)
):
