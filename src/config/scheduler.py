...
def periodic() -> BackgroundScheduler:
    ...
    background_scheduler.add_job(
        SeoulInspiredUsecase().daily_collect, "cron",
        id='9', day_of_week="*", hour=14, minute=20, second=0
    )
    background_scheduler.add_job(
        UnnieLooksUsecase().daily_collect, "cron",
        id='10', day_of_week="*", hour=14, minute=30, second=0
    )
    background_scheduler.add_job(
        OncurationUsecase().daily_collect, "cron",
        id='10', day_of_week="*", hour=14, minute=40, second=0
    )
